...
const uriBase = 'https://face-access.cognitiveservices.azure.com/face/v1.0/detect';
...
module.exports.faceDetect = async function faceDetect(imageUrl){
...
// const newFaceList = {
//   uri: 'https://face-access.cognitiveservices.azure.com/face/v1.0/facelists/testfacelist',
//   headers: {
//     'Content-Type': 'application/json',
//     'Ocp-Apim-Subscription-Key' : subscriptionKey
//   },
...
module.exports.newFace = async function newFace(imageUrl){

  const newFace = {
    uri: 'https://face-access.cognitiveservices.azure.com/face/v1.0/facelists/testfacelist/persistedFaces?detectionModel=detection_02',
    headers: {
      'Content-Type': 'application/json',
      'Ocp-Apim-Subscription-Key' : subscriptionKey
    },
  ...
  var addedFace = await request.post(newFace, (error, response, body) => {
    if (error) {
      console.log('Error: ', error);
      return;
    }
...
module.exports.faceVerify = async function faceVerify(faceId1, faceId2){
  const verify = {
    method: 'POST',
    uri: 'https://face-access.cognitiveservices.azure.com/face/v1.0/verify',
    headers: {
      'Content-Type': 'application/json',
      'Ocp-Apim-Subscription-Key' : subscriptionKey
    },
  ...
  var verifiedFaces = await request(verify).then((body) => {
    let jsonResponse = JSON.parse(body);
    // console.log(jsonResponse);
    return jsonResponse;
  }).catch( err =>{
...
module.exports.findSimilar =  async function findSimilar(comparingFace){
  const findSimilar = {
    method: 'POST',
    uri: 'https://face-access.cognitiveservices.azure.com/face/v1.0/findsimilars',
    headers: {
      'Content-Type': 'application/json',
      'Ocp-Apim-Subscription-Key' : subscriptionKey
    },
